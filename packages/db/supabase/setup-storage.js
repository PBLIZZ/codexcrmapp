...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error(
    'Missing Supabase URL or service role key. Make sure these are set in your .env file:'
  );
...
async function setupStorage() {
  try {
    ...
    const { data: bucketData, error: bucketError } =
      await supabase.storage.createBucket('contact-profile-photo', {
    ...
    if (readPolicyError) {
      ...
      console.log(
        'Policy may already exist or needs manual creation in the Supabase dashboard'
      );
    }
    ...
    console.log(
      '3. For the contact-profile-photo bucket, create these policies:'
    );
    console.log(
