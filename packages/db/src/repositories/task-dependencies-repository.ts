...
import { TaskDependency, TaskDependencyCreate, TaskDependencyModel } from '../models';
...
export class TaskDependenciesRepository {
   ...
   */
  async listDependenciesForTask(taskId: string): Promise<TaskDependencyModel[]> {
    ...
    return (data || []).map(dependency => TaskDependencyModel.fromDatabase(dependency as TaskDependency));
  }
   ...
   * @returns Array of task dependencies
   */
  async listTasksDependingOn(dependsOnTaskId: string): Promise<TaskDependencyModel[]> {
    const { data, error } = await this.supabase
      .from('task_dependencies')
      .select('*')
   ...
   */
  async getById(id: string): Promise<TaskDependencyModel | null> {
    const { data, error } = await this.supabase
      .from('task_dependencies')
      .select('*')
      .eq('id', id)
    ...
    return TaskDependencyModel.fromDatabase(data as TaskDependency);
  }
   ...
   */
  async create(dependency: TaskDependencyCreate): Promise<TaskDependencyModel> {
   ...
   * @param dependsOnTaskId The task ID that it depends on
   */
  async deleteDependency(taskId: string, dependsOnTaskId: string): Promise<void> {
    const { error } = await this.supabase
      .from('task_dependencies')
      .delete()
      .eq('task_id', taskId)
   ...
   */
  async hasAnyDependencies(taskId: string): Promise<boolean> {
   ...
   */
  async isADependencyForOthers(taskId: string): Promise<boolean> {
    const { count, error } = await this.supabase
      .from('task_dependencies')
      .select('*', { count: 'exact', head: true })
      .eq('depends_on_task_id', taskId);
   ...
   */
  async getDependencyIds(taskId: string): Promise<string[]> {
    const { data, error } = await this.supabase
      .from('task_dependencies')
   ...
   */
  async getDependentTaskIds(taskId: string): Promise<string[]> {
    const { data, error } = await this.supabase
      .from('task_dependencies')
      .select('task_id')
      .eq('depends_on_task_id', taskId);
   ...
   */
  async areAllDependenciesCompleted(taskId: string, tasksRepository: any): Promise<boolean> {
