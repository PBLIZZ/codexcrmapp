...
function findFilesWithIssues() {
  console.warn(`${colors.bold}${colors.blue}Finding files with ESLint issues...${colors.reset}`);
  
  const output = runCommand('npx eslint . --ext .ts,.tsx,.js,.jsx -f json', true);
  let files = [];
  ...
  try {
    ...
    console.error(`${colors.red}Error parsing ESLint output:${colors.reset}`, error);
  }
  ...
  for (const file of files) {
    try {
      ...
      if (content !== updatedContent) {
        ...
        console.warn(`${colors.green}Fixed console.log statements in:${colors.reset} ${file}`);
      }
    } catch (error) {
      console.error(`${colors.red}Error fixing console statements in ${file}:${colors.reset}`, error);
    }
  }
}
...
function fixUnusedVariables() {
  ...
  try {
    ...
    runCommand('npx eslint . --ext .ts,.tsx --rule \'{"@typescript-eslint/no-unused-vars": ["error", {"vars":"all","args":"after-used"}]}\' --fix');
    console.warn(`${colors.green}Unused variables fixed automatically.${colors.reset}`);
  } catch (error) {
...
function fixImportOrder() {
  ...
  try {
    ...
    runCommand('npx eslint . --ext .ts,.tsx,.js,.jsx --rule \'{"import/order": ["error", {"groups": ["builtin", "external", "internal", "parent", "sibling", "index"]}]}\' --fix');
    console.warn(`${colors.green}Import order fixed automatically.${colors.reset}`);
  } catch (error) {
    console.error(`${colors.red}Error fixing import order:${colors.reset}`, error);
  }
}
...
function fixServerTsUnusedVars() {
  ...
  try {
    if (fs.existsSync(serverTsPath)) {
      ...
      console.warn(`${colors.green}Fixed unused error variables in server.ts${colors.reset}`);
    }
  } catch (error) {
...
function fixMainLayoutUnusedImports() {
  ...
  const mainLayoutPath = path.join(rootDir, 'apps/web/components/layout/MainLayout.tsx');
  ...
  try {
    if (fs.existsSync(mainLayoutPath)) {
      ...
      console.warn(`${colors.green}Fixed unused imports in MainLayout.tsx${colors.reset}`);
    }
  } catch (error) {
    console.error(`${colors.red}Error fixing MainLayout.tsx:${colors.reset}`, error);
  }
}
...
function fixDashboardContentAnyTypes() {
  ...
  const dashboardContentPath = path.join(rootDir, 'apps/web/components/dashboard/DashboardContent.tsx');
  ...
  try {
    if (fs.existsSync(dashboardContentPath)) {
      ...
      console.warn(`${colors.green}Fixed any types in DashboardContent.tsx${colors.reset}`);
    }
  } catch (error) {
    console.error(`${colors.red}Error fixing DashboardContent.tsx:${colors.reset}`, error);
  }
}
...
async function main() {
  ...
  console.warn(`${colors.blue}Found ${files.length} files with ESLint issues.${colors.reset}`);
  ...
  console.warn(`For any remaining issues, please refer to the ESLINT-CLEANUP-GUIDE.md file.`);
}

main().catch(error => {
  console.error(`${colors.red}Error running batch ESLint fix script:${colors.reset}`, error);
});
