...
function fixUnusedVariables(content) {
  ...
  const eslintDisableRegex = /\/\/ eslint-disable-next-line @typescript-eslint\/no-unused-vars/g;
    ...
    for (const result of results) {
      for (const message of result.messages) {
        if (message.ruleId === '@typescript-eslint/no-unused-vars') {
          const match = message.message.match(/'([a-zA-Z0-9_]+)' is defined but never used/);
          if (match && match[1]) {
...
function fixImportOrder(content) {
  ...
  if (importLines.length > 0 && otherLines.length > 0 && otherLines[0].trim() !== '') {
    otherLines.unshift('');
  }
...
function fixTypeIssues(content, filePath) {
  ...
  updatedContent = updatedContent.replace(/: any(\s*)(=|;|\))/g, ': Record<string, unknown>$1$2');
  updatedContent = updatedContent.replace(/as any(\s*)(;|\))/g, 'as Record<string, unknown>$1$2');
  
  // For function parameters that are objects
  updatedContent = updatedContent.replace(/\(([^)]*): any(\s*)\)/g, '($1: Record<string, unknown>$2)');
