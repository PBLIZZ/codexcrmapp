...
function fixUnusedVariablesInFile(filePath, replacements) {
  try {
    console.warn(`${colors.cyan}Fixing unused variables in: ${filePath}${colors.reset}`);
    
    if (!fs.existsSync(filePath)) {
      console.warn(`${colors.yellow}File not found: ${filePath}${colors.reset}`);
      return false;
    }
    ...
    for (const replacement of replacements) {
      ...
      if (content.includes(pattern)) {
        ...
        modified = true;
        console.warn(`${colors.green}Fixed: ${pattern} -> ${replaceWith}${colors.reset}`);
      }
    }
    
    if (modified) {
      fs.writeFileSync(filePath, content);
      console.warn(`${colors.green}Successfully updated: ${filePath}${colors.reset}`);
      return true;
    } else {
      console.warn(`${colors.yellow}No changes needed in: ${filePath}${colors.reset}`);
      return false;
    }
  } catch (error) {
    console.error(`${colors.red}Error fixing unused variables in ${filePath}:${colors.reset}`, error);
    return false;
  }
}
...
const filesToFix = [
  {
    path: path.join(rootDir, 'apps/web/app/reset-password/ResetPasswordContent.tsx'),
    replacements: [
      {
      ...
      },
      {
        ...
        replacement: 'const { data: _data, error } = await supabase.auth.updateUser({'
      }
    ]
  },
  {
...
main().catch(error => {
  console.error(`${colors.red}Error running fix remaining unused variables script:${colors.reset}`, error);
});
