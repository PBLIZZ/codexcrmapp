...
function takeStock() {
  ...
  try {
    ...
    console.error(
      `${colors.red}Error taking stock of ESLint issues:${colors.reset}`,
      error
    );
    return [];
  }
...
function fixUnusedVars() {
  ...
  try {
    ...
    console.warn(
      `${colors.green}Unused variables and imports fixed automatically.${colors.reset}`
    );
    return true;
  } catch (error) {
    console.error(
...
function addExplicitReturnTypes() {
  ...
  try {
    ...
    const codemodPath =
      'node_modules/@typescript-eslint/utils/dist/ast-utils/explicitReturnType.js';
    if (!fs.existsSync(codemodPath)) {
      console.warn(
        `${colors.yellow}Codemod script not found at ${codemodPath}.${colors.reset}`
      );
      console.warn(
        `${colors.yellow}You may need to install @typescript-eslint/utils or check the path.${colors.reset}`
      );
      return false;
    }
    ...
    console.warn(
      `${colors.green}Explicit function return types added.${colors.reset}`
    );
    return true;
  } catch (error) {
    console.error(
      `${colors.red}Error adding explicit return types:${colors.reset}`,
      error
    );
    return false;
  }
...
function tackleExplicitAny() {
  ...
  try {
    ...
    const output = runCommand(
      'npx eslint . --ext .ts,.tsx --rule \'{"@typescript-eslint/no-explicit-any": ["error"]}\' -f table | grep -i "no-explicit-any"'
    );
    console.warn(output);
  } catch (error) {
    console.warn(
      `${colors.yellow}Could not list files with explicit 'any' types.${colors.reset}`
    );
  }
}
...
function setupCIGuardrail() {
  ...
  if (!fs.existsSync(eslintConfigPath)) {
    console.warn(
      `${colors.yellow}ESLint config file not found at ${eslintConfigPath}.${colors.reset}`
    );
    return false;
  }
...
async function main() {
  ...
  console.warn(
    `This script will help you systematically fix ESLint issues in your codebase.`
  );
  ...
  console.warn(
    `Follow the recommendations above to systematically clean up your codebase.`
  );
...
main().catch((error) => {
  console.error(
    `${colors.red}Error running ESLint cleanup script:${colors.reset}`,
    error
  );
  rl.close();
});
