...
import { parseCsvFile, validateCsvData, CsvValidationError, ValidatedContactData } from '@/lib/csv-utils'; // Removed ParsedCsvRow, Added ValidatedContactData
...
export default function CsvUploadTestPage() {
  ...
  const [parsedData, setParsedData] = useState<ValidatedContactData[] | null>(null);
  const [parsingErrors, setParsingErrors] = useState<Array<{ message: string; code?: string; type?: string; row?: number }> | null>(null);
  const [rowCount, setRowCount] = useState<number | null>(null);
  const [csvValidationErrors, setCsvValidationErrors] = useState<CsvValidationError[] | null>(null);
  const [isImporting, setIsImporting] = useState<boolean>(false);
  const [importFeedback, setImportFeedback] = useState<string>('');
  const [importResult, setImportResult] = useState<ImportCsvDataResponse | { error: string } | null>(null); // To store backend response
  ...
  const handleFilesAccepted = async (files: FileWithPath[]) => {
    ...
    if (files.length > 0) {
      ...
      try {
        ...
        setParsingErrors(parsedResult.errors.length > 0 ? parsedResult.errors.map(e => ({...e, row: e.row ?? -1 })) : null); // Ensure row is always number
        ...
        setCsvValidationErrors(validationResult.errors.length > 0 ? validationResult.errors : null);
        ...
        } else if (parsedResult.errors.length === 0 && validationResult.validatedData.length > 0) {
        ...
        } else if (parsedResult.errors.length === 0 && validationResult.validatedData.length === 0 && parsedResult.data.length > 0) {
        ...
        setFeedbackMessage(`Error processing "${file.name}": ${(error as Error).message}`);
        setParsingErrors([{ message: (error as Error).message, row: 0, code: 'ProcessingError', type: 'General' }]);
        setParsedData(null);
        setCsvValidationErrors(null);
      }
    } else {
  ...
  const importMutation = api.contacts.importCsvData.useMutation({
    ...
    onError: (error: TRPCClientErrorLike<AppRouter>) => { // Use TRPCClientErrorLike
  ...
  const handleImportContacts = () => {
    if (parsedData && (!csvValidationErrors || csvValidationErrors.length === 0)) {
      ...
      const contactsWithOriginalIndex = parsedData.map((contact, index) => ({ ...contact, originalIndex: index }));
      importMutation.mutate({ contacts: contactsWithOriginalIndex });
    } else {
      setImportFeedback('Cannot import: No valid data or validation errors exist.');
    }
  };
    ...
    <div className="container mx-auto p-8">
      ...
      <div className="max-w-xl mx-auto">
        <p className="mb-4 text-muted-foreground">
          This page demonstrates the integration of the <code>CsvUpload</code> component.
          Try uploading a CSV file (max 10MB).
        </p>
      ...
      {feedbackMessage && (
        <div className="mt-6 p-4 border rounded-md bg-secondary/50">
          <h2 className="text-lg font-semibold mb-2">Parent Component Feedback:</h2>
          <p>{feedbackMessage}</p>
          {uploadedFile && (
          ...
          {rowCount !== null && (
            <p className="mt-2 text-sm"><strong>Rows Parsed (excluding header):</strong> {rowCount}</p>
          )}
          {parsingErrors && parsingErrors.length > 0 && (
            <div className="mt-3">
              <h3 className="text-md font-semibold text-destructive mb-1">Parsing Errors:</h3>
              <ul className="list-disc list-inside text-sm text-destructive/90 bg-destructive/10 p-2 rounded-md">
                {parsingErrors.map((err, index) => (
                  <li key={index}>
                    Row {err.row !== undefined ? err.row + 1 : 'N/A'}: {err.message} {err.code && `(${err.code})`}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {parsedData && parsedData.length > 0 && (
            <div className="mt-3">
              <h3 className="text-md font-semibold mb-1">Parsed Data Preview (first 5 rows):</h3>
          ...
          {csvValidationErrors && csvValidationErrors.length > 0 && (
            <div className="mt-3">
              <h3 className="text-md font-semibold text-yellow-700 mb-1">CSV Validation Issues:</h3>
              <ul className="list-disc list-inside text-sm text-yellow-700 bg-yellow-50 p-2 rounded-md">
                {csvValidationErrors.map((err, index) => (
                  <li key={`val-${index}`}>
                    {err.type === 'header' ? `Header Error: ${err.message}` :
          ...
          {parsedData && parsedData.length > 0 && (!csvValidationErrors || csvValidationErrors.length === 0) && (
            <div className="mt-6">
              <Button 
                onClick={handleImportContacts} 
                disabled={isImporting || !parsedData || (!!csvValidationErrors && csvValidationErrors.length > 0)}
              >
          ...
          {importFeedback && (
            <div className={`mt-4 p-3 rounded-md ${importResult && 'error' in importResult ? 'bg-destructive/20 text-destructive' : 'bg-green-100 text-green-700'}`}>
              <p className="font-semibold">Import Status:</p>
              <p>{importFeedback}</p>
              {importResult && (
                <div className="mt-4 p-3 border rounded-md bg-muted/50">
                  <h3 className="text-md font-semibold mb-1">Import Results:</h3>
                  {'error' in importResult ? (
                    <p className="text-sm text-red-600">Error Detail: {importResult.error}</p>
                  ) : (
                    <>
                      <p className="text-sm">Successfully imported: {importResult.successCount} contacts.</p>
                      <p className="text-sm">Skipped: {importResult.skippedCount} contacts.</p>
                      <p className="text-sm">Failed to import (errors): {importResult.errorCount} contacts.</p>
                      {importResult.results && importResult.results.length > 0 && (
                        <details className="mt-2 text-xs">
                          <summary>Show detailed results ({importResult.results.length} rows processed)</summary>
                          <ul className="max-h-40 overflow-y-auto list-disc pl-5 mt-1 space-y-1">
                            {importResult.results.map((item) => (
                              <li key={item.originalIndex} className={
                                item.status === 'imported' ? 'text-green-700' :
                                item.status === 'skipped' ? 'text-yellow-700' :
                                item.status === 'error' ? 'text-red-700' : ''
                              }>
                                Row {item.originalIndex + 1}: {item.status.toUpperCase()} - {item.message}
                                {item.email && ` (Email: ${item.email})`}
                                {item.errorDetails && <span className="block text-red-500">Error: {item.errorDetails}</span>}
                              </li>
                            ))}
                          </ul>
                        </details>
