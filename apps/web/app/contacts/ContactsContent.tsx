...
import { Plus, Search, SlidersHorizontal, Tag, X } from 'lucide-react';
...
export function ContactsContent({ initialGroupId }: { initialGroupId?: string } = {}) {
  ...
  // const [isSidebarOpen, setIsSidebarOpen] = useState(true); // Commented out - reserved for future sidebar functionality
  ...
  const handleDeleteClick = (contactId: string) => {
    ...
    if (
      window.confirm('Are you sure you want to delete this contact? This action cannot be undone.')
    ) {
              ...
              {selectedGroupId && (
                <div className='flex items-center'>
                  <div className='bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm font-medium flex items-center mr-4'>
                    {groups.find((g: { id: string; name: string }) => g.id === selectedGroupId)
                      ?.name || 'Group'}
                    <button
                      onClick={() => {
