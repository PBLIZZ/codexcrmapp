...
import { ContactGroupsSection } from '../groups/ContactGroupsSection';

import {
  Alert,
  AlertDescription,
  AlertTitle,
  AvatarImage as CustomAvatarImage,
  Badge,
  Button,
  Card,
  CardContent,
...
export function ContactDetailView({ contactId }: { contactId: string }) {
  ...
  useEffect(() => {
    if (contact) {
      reset({
        ...
        enriched_data: contact.enrichedData ?? null,
      });
    }
  }, [contact, reset]);
  ...
  const handleDeleteContact = () => {
    deleteMutation.mutate({ contactId: contactId });
  };
          ...
          {contact.enrichmentStatus && (
            <div className='absolute top-4 right-4'>
              <Badge
                ...
                variant={
                  contact.enrichmentStatus === ENRICHMENT_STATUS.COMPLETED
                    ? 'default'
                    : contact.enrichmentStatus === ENRICHMENT_STATUS.PENDING
                      ? 'outline'
                      : contact.enrichmentStatus === ENRICHMENT_STATUS.FAILED
                        ? 'destructive'
                        : 'secondary'
                }
              >
        ...
        <TabsContent value={TABS.OVERVIEW} className='space-y-6'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Complete profile information for {contact.fullName}</CardDescription>
            </CardHeader>
            <CardContent className='space-y-6'>
              ...
              {contact.enrichedData && Object.keys(contact.enrichedData).length > 0 && (
                <div>
                  <h3 className='text-lg font-medium mb-2'>Enriched Information</h3>
                  <Separator className='mb-4' />
                  <div className='bg-muted/50 p-4 rounded-md'>
                    <pre className='text-sm whitespace-pre-wrap'>
                      {JSON.stringify(contact.enrichedData, null, 2)}
                    </pre>
                  </div>
                </div>
              )}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className='sm:max-w-[600px] max-h-[90vh] overflow-y-auto'>
          <DialogHeader>
            ...
            <DialogDescription>Update information for {contact.fullName}</DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className='sm:max-w-[425px]'>
          ...
          <div className='py-4'>
            <p className='text-sm text-muted-foreground'>
              This will permanently delete <span className='font-semibold'>{contact.fullName}</span>{' '}
