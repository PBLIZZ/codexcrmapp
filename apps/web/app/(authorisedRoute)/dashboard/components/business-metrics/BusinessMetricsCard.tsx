...
import { ArrowRight, Calendar, AlertTriangle, TrendingUp } from 'lucide-react';
...
export function BusinessMetricsCard({ className }: BusinessMetricsCardProps) {
  ...
  const {
    ...
    isLoading,
    error,
  } = api.dashboard.summary.useQuery(
    {},
    {
      retry: false,
    }
  ...
  const journeyStageData =
    contactMetrics.journeyStageDistribution.map((stage: any, index: number) => {
  ...
  const sessionTrendData =
    sessionMetrics.sessionTrend.map((item: any) => ({
    ...
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ${className}`}>
      ...
      <Card className='md:col-span-2'>
        <CardHeader className='text-primary'>
          <CardTitle className='text-primary'>Well-being Customer Journey</CardTitle>
      ...
      <Card className='md:col-span-1'>
        <CardHeader className='text-primary'>
          <CardTitle className='text-primary'>Session Trend</CardTitle>
          <CardDescription className='text-primary'>Your wellness journey impact</CardDescription>
        </CardHeader>
        <CardContent className='text-primary'>
          <div className='h-80'>
