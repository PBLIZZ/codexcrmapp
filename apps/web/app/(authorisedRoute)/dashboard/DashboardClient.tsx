...
const OmniBot = dynamic(
  () => import('../../../components/omni-bot/OmniBot').then((mod) => ({ default: mod.OmniBot })),
  {
    loading: () => null,
    ssr: false,
  }
...
class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className='p-6'>
          <Alert className='p-6' variant='danger'>
            <AlertCircle className='h-4 w-4' />
            <AlertTitle className='p-6'>Dashboard Error</AlertTitle>
