      ...
      <div className="space-y-6">
        <div className="bg-green-50 dark:bg-green-950/20 p-6 rounded-lg">
          ...
          <p className="text-muted-foreground mb-6">
            Create engaging quizzes to qualify leads, segment your audience, or educate clients about your services. 
            Interactive quizzes boost engagement and provide valuable insights about your audience.
          </p>
                    ...
                    {question.options.map((option, oIndex) => (
                      <div key={oIndex} className="flex items-center gap-2">
                        <Button 
                          variant="ghost" 
                          size="icon" 
                          className={`h-6 w-6 ${question.correctAnswer === oIndex ? 'text-green-600' : 'text-muted-foreground'}`}
      ...
      <div className="space-y-6">
        <Card className="border-green-100 dark:border-green-900/50">
          ...
          <CardContent className="pt-6">
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">Discover more about yourself with our interactive quiz</p>
              </div>
                ...
                <div className="border rounded-md p-4 bg-white dark:bg-green-950/10">
                  ...
                  <div className="space-y-2">
                    {questions[0].options.map((option, index) => (
                      <div 
                        key={index} 
                        className={`p-3 rounded-md border cursor-pointer hover:bg-green-50 dark:hover:bg-green-900/20 ${index === 0 ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : ''}`}
                      >
                  ...
                  <div className="flex justify-between mt-4">
                    ...
                    <Button size="sm" className="bg-green-600 hover:bg-green-700">Next</Button>
                  </div>
                </div>
              ) : (
          ...
          <CardFooter className="flex justify-between border-t pt-4">
            <Button variant="outline" size="sm" asChild>
              <Link href={{ pathname: "/marketing/quiz/templates" }}>
                Browse Templates
              </Link>
            </Button>
            <Button variant="outline" size="sm" className="bg-green-50 hover:bg-green-100 dark:bg-green-950/20 dark:hover:bg-green-900/30" asChild>
        ...
        <div className="bg-indigo-50 dark:bg-indigo-950/20 p-4 rounded-lg">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Use quiz results to recommend the perfect membership tier for each client. Our membership tools help you create compelling programs that convert quiz takers into loyal members.
          </p>
          <Button variant="link" className="text-indigo-600 dark:text-indigo-400 p-0 h-auto mt-2" onClick={() => document.querySelector('[value="membership"]')?.dispatchEvent(new MouseEvent('click'))}>
