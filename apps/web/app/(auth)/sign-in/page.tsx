  ...
  const handleMagicLinkLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : 'An error occurred sending the magic link'
      );
      setMessageType('error');
    } finally {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          queryParams:
            provider === 'google'
              ? { access_type: 'offline', prompt: 'select_account', hd: '*' }
              : undefined,
        },
      });
      ...
      if (!data || !data.url) {
        ...
        setMessage(
          `Error starting ${provider} sign in - no redirect URL returned`
        );
        setMessageType('error');
        setIsOauthLoading(null);
        return;
      ...
      console.warn(
        `${provider} OAuth initiated successfully, redirecting to:`,
        data.url
      );
              ...
              <Button
                ...
                disabled={
                  authMethod === 'password'
                    ...
                    : isMagicLinkLoading || !!isOauthLoading
                }
              >
                {authMethod === 'password'
                  ? isLoading
