...
export function Navbar({ user }: NavbarProps) {
  ...
  const currentSectionColors = sectionColors[currentSectionPath] || sectionColors["/dashboard"];
    ...
    <nav className="sticky top-0 z-40 border-b bg-white shadow-md">
      <div className="container mx-auto flex h-16 items-center justify-between py-2">
        ...
        <div className="md:hidden">
            <div className={`px-3 py-1.5 rounded-md text-sm font-semibold ${currentSectionColors.accent} ${currentSectionColors.text} shadow-sm`}>
              {mainNavItems.find(item => item.href === currentSectionPath)?.title || "Dashboard"}
            </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-4 lg:gap-6">
            {mainNavItems.map((item) => {
              ...
              const sectionColor = sectionColors[item.href] || sectionColors["/dashboard"];
        ...
        <div className="flex items-center gap-2 sm:gap-3">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild><Link href="/settings/general" className="w-full">General</Link></DropdownMenuItem>
                <DropdownMenuItem asChild><Link href="/settings/appearance" className="w-full">Appearance</Link></DropdownMenuItem>
                <DropdownMenuItem asChild><Link href="/settings/notifications" className="w-full">Notifications</Link></DropdownMenuItem>
                <DropdownMenuItem asChild><Link href="/settings/security" className="w-full">Security</Link></DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56" align="end" forceMount>
                ...
                <DropdownMenuSeparator />
                <SignOutButton /> {/* Use the dedicated SignOutButton component */}
              </DropdownMenuContent>
            </DropdownMenu>
        </div>
      </div>
