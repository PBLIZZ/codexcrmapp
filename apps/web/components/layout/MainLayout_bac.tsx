...
import { ContactsSidebar } from "../../app/contacts/ContactsSidebarOriginalWithGroupsAndContactCounts";
// Import other sidebar components as you create them
import { DashboardSidebar } from "./DashboardSidebar";
import { GroupsAdminSidebar } from "./GroupsAdminSidebar";
import { Navbar } from "./Navbar_bac"; // Your consolidated Navbar
...
const mainNavItemsForSheet = [
  ...
  { title: "Messages", href: "/messages", icon: MessageSquare, disabled: false },
...
export function MainLayout({ children, user }: MainLayoutProps) {
  ...
  const { data: contactsData } = api.contacts.list.useQuery(undefined, {
  ...
  const getCurrentSectionPath = (): SectionPath => {
    for (const item of mainNavItemsForSheet) { // Use a consistent nav item list
  ...
  const currentSectionTitle = mainNavItemsForSheet.find(item => item.href === currentSectionPath)?.title || "Dashboard";
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gray-50"> {/* Added a light bg to body */}
