...
export function parseCsvFile(
  file: File,
  options?: Omit<Papa.ParseLocalConfig<ParsedCsvRow, File>, 'complete' | 'error'>
): Promise<ParseCsvFileResult> {
  return new Promise((resolve, reject) => {
    const papaConfig: Papa.ParseLocalConfig<ParsedCsvRow, File> = {
      // Default settings that can be overridden by options
      header: true,
      skipEmptyLines: true,
      dynamicTyping: true,
      delimiter: ',', // Explicitly set CSV delimiter to comma (semicolons are used within fields)
...
const REQUIRED_HEADERS: string[] = ['Full Name', 'Email'];
...
export function validateCsvData(parsedResult: ParseCsvFileResult): {
  ...
  parsedResult.data.forEach((originalRow, index) => {
    ...
    // Check for email presence and basic format (support both 'email' and 'Email')
    ...
    // Handle full_name (support both 'full_name' and 'Full Name')
    const fullNameValue = row['full_name'] ?? row['Full Name'];
    if (
      fullNameValue === null ||
      ...
      if (emailIsValid && typeof emailValue === 'string') {
        ...
        if (!emailIsValid) {
          errors.push({
            row: csvRowNumber,
            field: 'full_name',
            message: 'Full name is missing and cannot be derived (invalid or missing email).',
            type: 'data',
          });
        }
      }
    ...
    // Validate website if present (support both 'website' and 'Website')
    const websiteValue = row['website'] ?? row['Website'];
    if (websiteValue !== null && websiteValue !== undefined && String(websiteValue).trim() !== '') {
      const trimmedValue = String(websiteValue).trim();
      // Accept common placeholder values and skip validation for them
      const placeholderValues = ['n/a', 'na', 'none', '?', '-', 'null', 'undefined', ''];
      const isPlaceholder = placeholderValues.includes(trimmedValue.toLowerCase());
      
      if (!isPlaceholder && typeof websiteValue === 'string' && !URL_REGEX.test(trimmedValue)) {
        errors.push({
          row: csvRowNumber,
          field: 'website',
          message: `Invalid URL format for website: "${trimmedValue}". Use format like "example.com" or "https://example.com".`,
          type: 'data',
        });
      }
    }
