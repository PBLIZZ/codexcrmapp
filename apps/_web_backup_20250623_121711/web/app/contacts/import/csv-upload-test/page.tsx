...
export default function CsvUploadTestPage() {
  ...
  const [isImporting, _setIsImporting] = useState<boolean>(false);
  ...
  const handleFilesAccepted = async (files: FileWithPath[]) => {
    ...
    if (files.length > 0) {
      const file = files[0];
      if (!file) {
        // Handle potential undefined file
        return;
      }
    ...
    <div className='container mx-auto p-8'>
      ...
      <div className='max-w-xl mx-auto'>
        <p className='mb-4 text-muted-foreground'>
          This page demonstrates the integration of the <code>CsvUpload</code> component. Try
          uploading a CSV file (max 10MB).
        </p>
        <CsvUpload
            ...
            <div className='mt-3'>
              ...
              <ul className='list-disc list-inside text-sm text-destructive/90 bg-destructive/10 p-2 rounded-md'>
                {parsingErrors.map((err, index) => (
                  <li key={index}>
                    Row {(err.row ?? -1) + 1}: {err.message} {err.code && `(${err.code})`}
                  </li>
                ))}
              </ul>
            </div>
                ...
                {csvValidationErrors.map((err, index) => (
                  <li key={`val-${index}`}>
                    {err.type === 'header'
                      ...
                      : `Row ${err.row ?? 'N/A'}${err.field ? ` (Field: ${err.field})` : ''}: ${
                          err.message
                        }`}
                  </li>
                ))}
                ...
                <div className='mt-4 p-3 border rounded-md bg-muted/50'>
                  ...
                  {'error' in importResult ? (
                    ...
                    <>
                      ...
                      <p className='text-sm'>
                        Failed to import (errors): {importResult.errorCount} contacts.
                      </p>
                      {importResult.results && importResult.results.length > 0 && (
                        <details className='mt-2 text-xs'>
                          <summary>
                            Show detailed results ({importResult.results.length} rows processed)
                          </summary>
                          <ul className='max-h-40 overflow-y-auto list-disc pl-5 mt-1 space-y-1'>
                            {importResult.results.map((item) => (
                              ...
                              >
                                Row {item.originalIndex + 1}: {item.status.toUpperCase()} -{' '}
                                {item.message}
